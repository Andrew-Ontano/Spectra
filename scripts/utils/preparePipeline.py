...
def main():
    ...
    parser.add_argument('-m', '--minimum-sequence-size', dest='minimum_size', type=int, help='Minimum sequence size to include in reports [1 bp]', default=1)
    ...
    parser.add_argument('--auto-stat', dest='auto_stat', type=float, default=1, help='Multiplier for SD or factor for IQR for automatic determination [default 1]')
    parser.add_argument('--raw-min', dest='raw_min', type=int, default=50, help='Jellyfish2 raw kmer minimum count to retain [default 100]')
    ...
    parser.add_argument('--mq-window', dest='mq_window', type=int, default=10000, help='Window and spacing width for kmer mass-query.py localization [default 200,000 bp]')
    ...
    parser.add_argument('--skip', dest='skip', type=int, default=0, help='Skip sections up to N. 1: Raw kmer counting, 2: Assembly kmer counting, 3: kmer comparison, 4: N-mer localization, 5: 3-mer localization, 6: report generation')
    ...
    with open(args.output, 'w') as f:
        ...
        if args.skip < 3:
            ...
            if args.auto_percentile:
                kmer_comp_cmd += f" --auto --auto-method {args.auto_method} --auto-stat {args.auto_stat}"
            kmer_comp_cmd += " -v"
            f.write(kmer_comp_cmd + "\n")

        # Percentile logic needed for localization and report
        if args.skip < 4 or args.skip < 6:
            if args.auto_percentile:
                ...
                f.write(f"else\n")
                if args.skip >= 3:
                    f.write(f"    echo \"Warning: {variables['prefix']}/{variables['prefix']}_kmer_comp_percentiles.txt not found. Using default percentiles.\"\n")
